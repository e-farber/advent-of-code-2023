 ...
 * history should be.
 * Part 2:
 * Analyze your report again, this time extrapolating the previous value for
...
int extrapolate_value(std::deque<int> sequence);
...
int main() {
    ...
    while (std::getline(inputFile, current_line)) {
        std::deque<int> current_sequence;
...
int extrapolate_value(std::deque<int> sequence) {
    ...
    sequence.push_front(sequence[0] - extrapolate_value(next_sequence));
