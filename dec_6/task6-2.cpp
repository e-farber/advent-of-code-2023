...
/**
 * December 6th, task 6-2
 * adventofcode.com/2023/day/6
 * 
 * Short summary:
 * Your puzzle input lists the time allowed for a single toy boat race
 * (ignore the spaces) and also the best distance ever recorded in that race.
 * To guarantee you win the grand prize, you need to make sure you go farther
 * in that race than the current record holder. 
 ...
 * Determine the number of ways you can beat the record in that race.
*/
...
size_t concatenate(const std::vector<size_t> &input_vector);
...
int main() {
    ...
    */
    float total_time_n {static_cast<float>(concatenate(race_data.time    ))};
    float distance_d   {static_cast<float>(concatenate(race_data.distance))};
...
size_t concatenate(const std::vector<size_t> &input_vector) {
    std::string vec_to_str;
    for (auto &number : input_vector) {
        vec_to_str.append(std::to_string(number));
    }
    auto iterator {vec_to_str.begin()};
    size_t result {get_next_number(iterator)};
    return result;
}
